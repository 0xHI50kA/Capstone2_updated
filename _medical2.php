...
<!--=========== MY CSS LINK START =============-->
	...
	<link rel="stylesheet" type="text/css" href="css/index21.css">
...
<!--================= Rutchy PROFILE START ==================-->
<div class="profile">

<?php
include('AAtabHealthCare/servicess/connection.php');

// Fetch Nurse Rutchy's profile from DB (assuming it's the latest or use specific `id` if needed)
$sql = "SELECT * FROM medical_clinics WHERE id = 2 LIMIT 1"; // or WHERE id = 1
$result = mysqli_query($conn, $sql);

if ($row = mysqli_fetch_assoc($result)) {
    $name = $row['name'];
    $img = $row['image_path']; // stored with 'uploads2/' already
...
<div class="row container-fluid dr">
    <div class="col-sm-12 col-lg-5 col-md-12 animate drImage">
        <img src="Admin2/uploads3/<?php echo htmlspecialchars(basename($img)); ?>" alt="Clinic Image">
    </div>
    <div class="col-sm-12 animate col-lg-5 col-md-12 drDescript">
        <p class="drName"><?php echo htmlspecialchars($name); ?></p>
        <p>
            <?php if (!empty($mapEmbed)): ?>
                <?php echo $mapEmbed; ?> <!-- Show raw iframe HTML -->
            <?php else: ?>
                <span style="color: red;">No map available.</span>
            <?php endif; ?>
        </p>
    </div>
</div>
...
<script type="text/javascript" src="javascript/_dental1.js"></script>
<script>
