...
if (isset($_GET['delete'])) {
    ...
    $getImage = $conn->prepare("SELECT image_path FROM nurse_profiles WHERE id = ?");
    ...
    $delete = $conn->prepare("DELETE FROM nurse_profiles WHERE id = ?");
...
// Fetch all nurse profiles
$data = [];
$result = $conn->query("SELECT * FROM nurse_profiles ORDER BY id DESC");
if ($result) {
...
<script>
    ...
    function performDelete() {
        ...
        setTimeout(() => {
            window.location.href = `crudLocator.php?delete=${id}`;
        }, 1500);
    }
