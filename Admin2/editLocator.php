...
$id = intval($_GET['id']);
$stmt = $conn->prepare("SELECT * FROM nurse_profiles WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE nurse_profiles SET name=?, image_path=?, map_embed=? WHERE id=?");
    $stmt->bind_param("sssi", $name, $imagePath, $mapEmbed, $id);

    if ($stmt->execute()) {
        echo "<script>
            setTimeout(function() {
                window.location.href = 'crudLocator.php?updated=1';
            }, 1500);
        </script>";
    } else {
