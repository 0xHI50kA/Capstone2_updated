...
$id = intval($_GET['id']);
$stmt = $conn->prepare("SELECT * FROM medical_clinics WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE medical_clinics SET name=?, image_path=?, map_embed=? WHERE id=?");
    $stmt->bind_param("sssi", $name, $imagePath, $mapEmbed, $id);

    if ($stmt->execute()) {
        echo "<script>
            setTimeout(function() {
                window.location.href = 'crudLocator1.php?updated=1';
            }, 1500);
        </script>";
    } else {
...
<html>
<head>
    <title>Edit Medical Clinic</title>
    <style>
        body {
...
<div id="confirmModal">
    <div class="modal-content">
        ...
        <p>Are you sure you want to update this clinic?</p>
        <div>
